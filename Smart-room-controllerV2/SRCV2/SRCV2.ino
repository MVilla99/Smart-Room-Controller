...
void hueSet(){
  ...
  setHue(bulb, activated,colorshift[showTemp],100); //photoresistor value
  }
...
int luminosity(){
  ...
  bitSenseVal = map(pSenseVal, 0, 3000, 0, 255);  
  return bitSenseVal;
}
void pixRing(){
  int showTemp;
    ...
    pixel.setBrightness(100);//use photo resistor value
    pixel.show();
    }
    if (smartControl == true){
    pixel.setPixelColor(showTemp,colorShift[showTemp]);
